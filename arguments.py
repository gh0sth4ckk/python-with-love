...
def get_username() -> tuple[str, str] | tuple:
    ...
    if "-u1" in sys.argv or "-u2" in sys.argv:
        nick1_index: int = sys.argv.index("-u1")+1 if "-u1" in sys.argv else None
        nick2_index: int = sys.argv.index("-u2")+1 if "-u2" in sys.argv else None
        nick1: str = sys.argv[nick1_index] if nick1_index else "user1"
        nick2: str = sys.argv[nick2_index] if nick2_index else "user2"
        return nick1, nick2
    else:
        return ()
